"""
python .\test_co3d_camera.py 
--co3d_dir F:\Datasets\enhanced\codiff\ 
--co3d_anno_dir . 
--model_path .\model_tracker_fixed_e20.pt 
--category car 
--sequence "481_68744_133820" 
"""
import os
import torch
import numpy as np
...
def setup_args():
    ...
    parser.add_argument('--num_frames', type=int, default=10, help='Number of frames to use for testing (random sample per sequence)')
    parser.add_argument('--co3d_dir', type=str, required=True, help='Path to CO3D dataset')
    parser.add_argument('--co3d_anno_dir', type=str, required=None, help='Path to CO3D annotations', default='.')
    parser.add_argument('--seed', type=int, default=0, help='Random seed for reproducibility')
    parser.add_argument('--model_path', type=str, required=None, help='Path to the VGGT model checkpoint', default='./model_tracker_fixed_e20.pt')

    # NEW: category / sequence selection
    parser.add_argument('--category', type=str, default=None,
                        help='Optional: evaluate only this CO3Dv2 category (e.g. "car"). If omitted, evaluates default SEEN_CATEGORIES.')
    parser.add_argument('--sequence', type=str, default=None,
                        help='Optional: evaluate only this sequence/scene name within the category (e.g. "seq_0001"). Requires --category if you want a specific sequence.')
...
def main():
    ...
    # NEW: 사용자 지정 카테고리가 주어지면 그 카테고리만 평가
    if args.category is not None:
        if args.debug:
            print(f"[WARN] --debug active; ignoring --category {args.category}")
        else:
            SEEN_CATEGORIES = [args.category]
    ...
    for category in SEEN_CATEGORIES:
        ...
        # NEW: sequence(시퀀스) 지정 시 해당 시퀀스만 평가
        seq_names = sorted(list(annotation.keys()))
        if args.sequence is not None:
            if args.sequence in annotation:
                seq_names = [args.sequence]
            else:
                print(f"[WARN] sequence {args.sequence} not found in category {category}; skipping this category.")
                continue
